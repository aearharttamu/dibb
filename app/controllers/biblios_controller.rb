class BibliosController < ApplicationController
  ...
  private
    ...
    def biblio_params
      params.permit(:id, :biblio_set_id, :item_as_appears, :item_number, :title, :descriptors, :date_as_appears, :year, :publisher_id, :provenance, :pub_number, :size, :binding, :pagination, :unnumbered_pages, :contents, :category_as_appears, publication_places_json: [:id, :city, :state, :country], staff_json: [:id, :name, :role] )
    end
end
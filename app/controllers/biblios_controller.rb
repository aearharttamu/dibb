class BibliosController < ApplicationController
  ...
  private
    ...
    def biblio_params
      params.permit(:id, :biblio_set_id, :item_as_appears, :item_number, :title, :descriptors, :date_as_appears, :year, :publisher_id, :publisher_name, :provenance, :pub_number, :pub_number_type, :size, :binding, :pagination, :unnumbered_pages, :contents, :category_as_appears, publication_places_json: [:id, :city, :state, :country], staff_json: [:id, :person_id, :role_id, :new_person_name ], sequence_json: [:id, :biblio_id, :first_page_number_as_appears, :first_page_number, :final_page_number_as_appears, :final_page_number] )
    end
end